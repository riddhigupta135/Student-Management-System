...
<body>
	...
	<div class="container col-md-5">
		<div class="card">
			<div class="card-body">
					<form name="editBatchForm" action="updateBatch" method="post">
				...
				<input type="hidden" name="batchId" value="<c:out value='${batch.batchId}' />" />
				
				<fieldset class="form-group">
					<label>Day</label>
					<select name="day" id="day" >
					<option value="Monday" ${batch.day == "Monday" ? 'selected' : ' '}><c:out value="Monday" ></c:out></option> 
    				<option value="Tuesday" ${batch.day == "Tuesday" ? 'selected' : ' '}><c:out value="Tuesday" ></c:out></option>
    				<option value="Wednesday" ${batch.day == "Wednesday" ? 'selected' : ' '}><c:out value="Wednesday" ></c:out></option>
    				<option value="Thursday" ${batch.day == "Thursday" ? 'selected' : ' '}><c:out value="Thursday" ></c:out></option>
    				<option value="Friday" ${batch.day == "Friday" ? 'selected' : ' '}><c:out value="Friday" ></c:out></option>
  					</select>
				...
				<fieldset class="form-group">
					<label>Is Deleted</label> <input type="text"
						value="<c:out value="false" />" class="form-control"
						name="isDeleted" readonly="readonly">
				</fieldset>
				

				<button onclick = "validate()" type="submit" class="btn btn-success" style="background-color: inidigo">Save</button>
				</form>
			</div>
		</div>
	</div>
